...
class Vector3D
  ...
  def rotate(rotations)
    return rotate(rotations.to_h) if rotations.is_a?(Vector3D)
  ...
  def reverse_rotate(rotations)
    return reverse_rotate(rotations.to_h) if rotations.is_a?(Vector3D)

    result = self

    rotations.reverse_each do |axis, angle|
      result = Vector3D.rotate_around_axis(axis, angle).transpose * result
    end
